...
TEST_CASE("No false moves are made in 1000 games", "[Algorithm], [Heavy]"){
    ...
    for (int complete = 0; complete < 1000; complete++){
        bool result = result_queue.pop();
        REQUIRE(result);
        std::cout << complete + 1 << " out of 1000 games done" << std::endl;
    }
